...
def add_user(user_id: str, complites: str, admin: bool):
    with Session(autoflush=False, bind=engine) as db:
        new_user = User(user_id = user_id, admin = admin, complite_tasks = complites)
        db.add(new_user)
        db.commit()
        return "Чел успешно добавлен"
...
def update_user_complites(id_us: int, complites: str):
    with Session(autoflush=False, bind=engine) as db:
        data_user = db.query(User).filter(User.id==1).first()
        if data_user != None:
...
def delete_user():
    with Session(autoflush=False, bind=engine) as db:
        user_for_delete = db.query(User).filter(User.id==1).first()
